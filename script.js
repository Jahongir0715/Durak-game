...
renderGame();
const table = []; // пары атакующих и защищающихся карт
...
function playerAttack(cardIndex) {
  ...
  // После хода игрока ходит бот
  botDefend();
}

// Логика защиты бота — выбирает первую карту, которая может отбить
function botDefend() {
  ...
  // Найдем карту у бота, которая отбивает
  ...
  if (defenseCardIndex === -1) {
    ...
    // Бот берет карты (упростим — добавим карты со стола в руку бота)
    table.forEach(pair => {
...
// Простая функция проверки — может ли карта отбить другую
function canBeat(defenseCard, attackCard) {
  if (defenseCard.suit === attackCard.suit) {
    ...
    return defenseRankIndex > attackRankIndex;
  } else {
